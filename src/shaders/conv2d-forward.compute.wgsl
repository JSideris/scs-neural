...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let out_z = global_id.z; // batch_idx * output_channels + out_channel

    if (out_x >= params.output_width || out_y >= params.output_height) {
        return;
    }

    let batch_idx = out_z / params.output_channels;
    let out_c = out_z % params.output_channels;
    ...
    for (var kh: u32 = 0u; kh < params.kernel_size; kh++) {
        ...
        for (var kw: u32 = 0u; kw < params.kernel_size; kw++) {
            ...
            let input_base = ((batch_idx * params.input_height + u32(in_y)) * params.input_width + u32(in_x)) * params.input_channels;
            let weight_base = ((kh * params.kernel_size + kw) * params.input_channels) * params.output_channels + out_c;
    ...
    let z_value = acc + biases[out_c];
    let linear_idx = ((batch_idx * params.output_height + out_y) * params.output_width + out_x) * params.output_channels + out_c;
    
    z_values[linear_idx] = z_value;
